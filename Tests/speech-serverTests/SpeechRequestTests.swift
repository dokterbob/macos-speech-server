...
final class SpeechRequestTests: XCTestCase {
    ...
    func testDefaultVoice() {
        let req = SpeechRequest(model: "tts-1", input: "Hello", voice: nil, response_format: nil, speed: nil)
    ...
    func testExplicitVoice() {
        let req = SpeechRequest(model: "tts-1", input: "Hi", voice: "alba", response_format: nil, speed: nil)
        XCTAssertEqual(req.resolvedVoice, "alba")
    }

    func testExplicitFormat() {
        let req = SpeechRequest(model: "tts-1", input: "Hi", voice: nil, response_format: "pcm", speed: nil)
        XCTAssertEqual(req.resolvedFormat, "pcm")
    }

    func testExplicitSpeed() {
        let req = SpeechRequest(model: "tts-1", input: "Hi", voice: nil, response_format: nil, speed: 2.0)
        XCTAssertEqual(req.resolvedSpeed, 2.0, accuracy: 0.001)
    }

    func testExplicitSpeedAtMinimum() {
        let req = SpeechRequest(model: "tts-1", input: "Hi", voice: nil, response_format: nil, speed: 0.25)
        XCTAssertEqual(req.resolvedSpeed, 0.25, accuracy: 0.001)
    }

    func testExplicitSpeedAtMaximum() {
        let req = SpeechRequest(model: "tts-1", input: "Hi", voice: nil, response_format: nil, speed: 4.0)
